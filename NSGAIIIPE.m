...
% NSGA-III-PE
    ...
    currPop   = [lastPop,currPop];
    Zmin         = min(currPop(all(currPop.cons<=0,2)).objs,[],1);
    [currPop,FrontNo,~] = EnvironmentalSelection(currPop,Global.N,Z,Zmin);
    ...
    while Global.NotTermination(currPop)
        ...
        lastPop   = currPop;
        [currPop,FrontNo,Next] = EnvironmentalSelection([currPop,Offspring],Global.N,Z,Zmin);
        b=sum(Next);
...
function [Population,FrontNo,Next] = EnvironmentalSelection(Population,N,Z,Zmin)
